...
export async function GET(){const user=await getChatGPTUser();if(!user)return json({error:'로그인이 필요해요.'},401);try{await cleanupPhotos();const db=database();const rows=await db.prepare(`SELECT p.*, (SELECT COUNT(*) FROM cheers c WHERE c.post_id=p.id) AS cheers, EXISTS(SELECT 1 FROM cheers c WHERE c.post_id=p.id AND c.user_id=?) AS cheered FROM posts p WHERE p.club_id=? ORDER BY p.created_at DESC`).bind(user.userId,user.clubId).all();return json({posts:rows.results.map(p=>({...p,photo:p.photo_key?'/api/photos/'+p.id:null,photo_key:undefined}))})}catch(e){console.error('load posts failed',e);return json({error:'기록을 불러오지 못했어요. 다시 시도해 주세요.'},503)}}
export async function POST(request:Request){
 ...
 const name=user.fullName,kind=String(f.get('kind')||''),minutes=Number(f.get('minutes')),note=String(f.get('note')||'').trim();
 ...
 await db.prepare('INSERT INTO posts (id,user_id,name,kind,minutes,note,photo_key,created_at,club_id) VALUES (?,?,?,?,?,?,?,?,?)').bind(id,user.userId,name,kind,minutes,note,key,created,user.clubId).run();
